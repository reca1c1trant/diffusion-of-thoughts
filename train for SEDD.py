...
class SEDDModel(torch.nn.Module):
    ...
    def forward(self, z, gamma, embedding_matrix, bias_scale, x_selfcond, 
                selfcond_mask=None, x_embed=None, src_mask=None):
        if not z.is_cuda:
            z = z.cuda()
        if gamma is not None and not gamma.is_cuda:
            gamma = gamma.cuda()
        if x_selfcond is not None and not x_selfcond.is_cuda:
            x_selfcond = x_selfcond.cuda()
...
def main(**args):
    ...
    args.setdefault('model_path', 'models/sedd-medium')
    ...
    if args.auto_resume and os.path.exists('model.pt'):
        ...
        modules['model'].load_state_dict(state_dict, strict=False)
        logging.info(f'Loaded pretrained weights from {args.model_path}')
